...
const eventsThisYearCount = computed(() => {
    return allNews.value.filter(event => {
...
const totalEventsCount = computed(() => allNews.value.length);
...
<template>
  <div class="min-h-screen flex flex-col">
    ...
    <!-- Stats Boxes -->
    <div v-if="hasAnyRole(['Admin', 'Principal'])" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
    ...
    <div v-if="currentView === 'announcements'" class="w-full mt-8">
      ...
      <div v-if="filteredAnnouncements.length" class="space-y-6">
        ...
        >
          ...
          <div v-if="hasAnyRole(['Admin', 'Principal'])" class="absolute top-2 right-2 z-10">
            <Button
                icon="pi pi-trash"
